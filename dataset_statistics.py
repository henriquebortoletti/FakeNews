...
stop_words = read_text('stop_words.txt')
...
def date_histogram():
    ...
    for i, v in enumerate(df.values):
        if len(str(v)) > 3:
            plt.text(xlocs[i] -0.35, v + 12, str(v))
        elif len(str(v)) > 1:
            plt.text(xlocs[i] -0.25, v + 12, str(v))
        else:
            plt.text(xlocs[i]-0.1, v + 12, str(v))
    plt.show()
...
def tree_map(dict,title):
    ...
    wc = WordCloud(background_color="white", max_words=350,width=500,height=500,margin=1,max_font_size=45,colormap='Greys')
...
if __name__ == "__main__":
    words_in_categories(meta_true_info,full_true_info,'true')
    words_in_categories(meta_false_info,full_false_info,'fake')
